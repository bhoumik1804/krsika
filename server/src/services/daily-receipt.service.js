...
export const getDailyReceiptById = async (millId, id) => {
    const entry = await DailyReceipt.findOne({ _id: id, millId })
        .populate('createdBy', 'fullName email')
        .populate('updatedBy', 'fullName email')
    if (!entry) throw new ApiError(404, 'Daily receipt entry not found')
    return entry
}
...
export const updateDailyReceiptEntry = async (millId, id, data, userId) => {
    ...
    const entry = await DailyReceipt.findOneAndUpdate(
        { _id: id, millId },
        updateData,
        ...
        .populate('updatedBy', 'fullName email')
    if (!entry) throw new ApiError(404, 'Daily receipt entry not found')
    logger.info('Daily receipt entry updated', { id, millId, userId })
    return entry
}

export const deleteDailyReceiptEntry = async (millId, id) => {
    const entry = await DailyReceipt.findOneAndDelete({ _id: id, millId })
    if (!entry) throw new ApiError(404, 'Daily receipt entry not found')
    logger.info('Daily receipt entry deleted', { id, millId })
}

export const bulkDeleteDailyReceiptEntries = async (millId, ids) => {
    const result = await DailyReceipt.deleteMany({ _id: { $in: ids }, millId })
