...
import {
  fetchSackPurchases,
  createSackPurchase,
  fetchAllSackPurchases,
  fetchSackPurchaseByNumber,
  deleteSackPurchase,
  updateSackPurchase,
} from "../api/sackPurchasesApi";
...
export const useSackPurchases = () => {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      fetchSackPurchases({ page, pageSize, filters: columnFilters, sorting }),
...
export const useCreateSackPurchase = () => {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["sackPurchases"] }),
  });
};

export const useDeleteSackPurchase = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: deleteSackPurchase,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["sackPurchases"] });
    },
  });
...
export const useUpdateSackPurchase = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: updateSackPurchase,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["sackPurchases"] });
    },
  });
