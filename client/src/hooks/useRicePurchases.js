...
export const useRicePurchases = () => {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      fetchRicePurchases({ page, pageSize, filters: columnFilters, sorting }),
...
export const useCreateRicePurchase = () => {
  ...
  return useMutation({
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["ricePurchases"] }),
  });
};

export const useUpdateRicePurchase = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }) => updateRicePurchase(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ricePurchases"] });
    },
  });
...
export const useDeleteRicePurchase = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: deleteRicePurchase,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ricePurchases"] });
    },
  });
