...
export default function RemainingDOInfo() {
  ...
  const enrichedDOs = useMemo(() => {
    ...
    return doEntries.map((doEntry) => {
      ...
      const salesLifting = paddySales.reduce((sum, sale) => {
        if (sale.doEntries && Array.isArray(sale.doEntries)) {
          const matchingEntries = sale.doEntries.filter(
            (entry) => entry.doNumber === doNumber
          );
          const entrySum = matchingEntries.reduce((entryTotal, entry) => {
            return (
              entryTotal +
              (parseFloat(entry.dhanMota || 0) +
  ...
  if (doError) {
    return (
      <div className="flex flex-col items-center justify-center h-64 bg-card rounded-xl border">
        ...
        <p className="text-muted-foreground text-sm">
          {doErrorMsg?.message || "Something went wrong"}
        </p>
        <Button
          ...
          {balanceSort && (
            <div className="mb-4 flex items-center gap-2">
              <span className="text-sm text-muted-foreground">
                Sorted by उठाव शेष:{" "}
                {balanceSort === "low-to-high" ? "Low to High" : "High to Low"}
              </span>
              <Button
                variant="ghost"
                size="sm"
            ...
            <CardContent className="p-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-start gap-2">
                  <span className="text-cyan-600 font-semibold min-w-[80px]">
                    उठाव =
                  </span>
                  <div>
                    <div className="text-muted-foreground">
                      <span className="font-medium">1. शासकीय धान आवक</span>{" "}
                      (Government Paddy Inward) से related DO नंबर का धान नेट
                      वजन
                    </div>
                    <div className="text-muted-foreground mt-1">
                      <span className="font-medium">2. धान बिक्री</span> (Paddy
                      Sales) page से जिस DO नंबर को बेचा गया है उसकी मात्रा
                    </div>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-border/50">
                  <span className="text-pink-600 font-semibold min-w-[80px]">
                    उठाव शेष =
                  </span>
                  ...
                  <div className="flex gap-4">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-red-600 font-medium">
                        शेष मात्रा (Balance Quantity)
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <div className="h-3 w-3 rounded-full bg-green-500"></div>
                      <span className="text-green-600 font-medium">
                        शून्य शेष (Zero Balance)
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="divide-y divide-border/30">
                  ...
                  <div className="flex items-center gap-4 px-4 py-3.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-medium text-foreground">
                        {selectedDO.date
                          ? new Date(selectedDO.date).toLocaleDateString(
                              "en-IN",
                              {
                                day: "numeric",
                                month: "short",
