...
export function SalesDealsReport() {
    ...
    // Sample data matching the image categories for sales
        ...
        <>
            ...
            <Main className='flex flex-1 flex-col gap-4 sm:gap-6'>
                <div className='flex flex-wrap items-end justify-between gap-2'>
                    <div>
                        ...
                        <p className='text-muted-foreground'>
                            Track daily sales transactions by commodity
                        </p>
                    </div>
                ...
                <div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5'>
                    {salesDeals.map((deal, index) => (
                        <StatsCard
                            key={index}
                            title={deal.description}
                            value={`${deal.quantity.toFixed(2)} Qtl`}
                            icon={deal.icon || ShoppingCart}
                        />
                    ))}
                </div>
            </Main>
