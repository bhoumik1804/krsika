...
import {
    useDailyPurchaseDealList,
    useDailyPurchaseDealSummary,
} from './data/hooks'
...
export function PurchaseDealsReport() {
    ...
    // Fetch daily purchase deal data using the hook
    const {
        data: purchaseDealResponse,
        isLoading,
        isError,
    } = useDailyPurchaseDealList(millId || '', queryParams, {
        enabled: !!millId,
    })

    // Fetch summary data
    const { data: summaryResponse } = useDailyPurchaseDealSummary(
        millId || '',
    ...
    const purchaseDealsData = useMemo(() => {
        if (!purchaseDealResponse?.data) return []

        // Group by commodity and sum quantities
        const groupedData = purchaseDealResponse.data.reduce(
            (acc, item) => {
                ...
                <div className='flex flex-wrap items-end justify-between gap-2'>
                    <div>
                        <h2 className='text-2xl font-bold tracking-tight'>
                            {t('dailyReports.purchaseDealsPage.title')}
                        </h2>
                        <p className='text-muted-foreground'>
                            {t('dailyReports.purchaseDealsPage.subtitle')}
                        </p>
                    </div>
                    <DateRangePicker
                        date={dateRange}
                    ...
                    <CardContent>
                        {isLoading ? (
                            ...
                            <div className='py-10 text-center text-destructive'>
                                Failed to load purchase deal data
                            </div>
                        ) : (
                            <div className='overflow-x-auto'>
                                <Table>
                                    ...
                                    <TableBody>
                                        {purchaseDealsData.length === 0 ? (
                                            <TableRow>
                                                <TableCell
                                                    colSpan={2}
                                                    className='text-center text-muted-foreground'
                                                >
                                                    No purchase deal data found
                                                    for the selected date range
                                                </TableCell>
                                            </TableRow>
                                        ) : (
                                            purchaseDealsData.map(
                                                (deal, index) => (
                                                    <TableRow key={index}>
