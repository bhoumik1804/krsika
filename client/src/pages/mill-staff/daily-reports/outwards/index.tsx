...
import { useDailyOutwardList, useDailyOutwardSummary } from './data/hooks'
...
export function OutwardsReport() {
    ...
    // Fetch daily outward data using the hook
    const {
        data: outwardResponse,
        isLoading,
        isError,
    } = useDailyOutwardList(millId || '', queryParams, { enabled: !!millId })

    // Fetch summary data
    const { data: summaryResponse } = useDailyOutwardSummary(
        millId || '',
    ...
    const outwardsData = useMemo(() => {
        if (!outwardResponse?.data) return []

        // Group by item and sum quantities
        const groupedData = outwardResponse.data.reduce(
            (acc, item) => {
                const key = item.item
                if (!acc[key]) {
                ...
                <div className='flex flex-wrap items-end justify-between gap-2'>
                    <div>
                        <h2 className='text-2xl font-bold tracking-tight'>
                            {t('dailyReports.outwards.title')}
                        </h2>
                        <p className='text-muted-foreground'>
                            {t('dailyReports.outwards.subtitle')}
                        </p>
                    </div>
                    <DateRangePicker
                        date={dateRange}
                                    ...
                                    <TableBody>
                                        {outwardsData.length === 0 ? (
                                            <TableRow>
                                                ...
                                                >
                                                    No outward data found for
                                                    the selected date range
                                                </TableCell>
                                            </TableRow>
                                        ) : (
                                            outwardsData.map((row, index) => (
                                                <TableRow key={index}>
