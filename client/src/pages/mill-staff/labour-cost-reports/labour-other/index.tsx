...
export function LabourOtherReport() {
    ...
    const {
        ...
        isLoading,
        isError,
    } = useLabourOtherList(millId || '', queryParams, { enabled: !!millId })

    const labourOtherData = useMemo(() => {
                ...
                <div className='flex flex-wrap items-end justify-between gap-2'>
                    <div>
                        <h2 className='text-2xl font-bold tracking-tight'>
                            {t('reports.labourCostReports.other.title')}
                        </h2>
                        <p className='text-muted-foreground'>
                            {t('reports.labourCostReports.other.subtitle')}
                        </p>
                    </div>
                    <LabourOtherPrimaryButtons />
                </div>
                <LabourOtherTable
                    data={labourOtherData}
                    search={search}
                    navigate={navigate}
                    isLoading={isLoading}
                    isError={isError}
