...
const generateDummyResponse = ({ page = 1, pageSize = 10 }) => {
  ...
  return {
    ...
    data: {
      otherPurchases: paginatedData,
      totalOtherPurchases: total,
      pageSize,
      currentPage: page,
      totalPages,
      hasPrev: page > 1,
...
export const fetchOtherPurchases = async ({
  page = 1,
  pageSize = 10,
  filters = [],
  sorting = [],
...
export const updateOtherPurchase = async (id, purchaseData) => {
  try {
    const data = await apiClient.put(`/purchases/other/${id}`, purchaseData);
    return data;
  } catch (error) {
    ...
    return {
      success: true,
      message: "Other Purchase updated successfully (SIMULATED)",
      data: { ...purchaseData, _id: id },
    };
  }
};

export const deleteOtherPurchase = async (id) => {
  try {
    const data = await apiClient.delete(`/purchases/other/${id}`);
    return data;
  } catch (error) {
    ...
    return {
      success: true,
      message: "Other Purchase deleted successfully (SIMULATED)",
      data: { _id: id },
    };
  }
};
