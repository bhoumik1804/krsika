...
function Filter({ column }) {
  ...
  // Date filter with calendar and manual input
  if (filterVariant === "date") {
    const [open, setOpen] = React.useState(false);
    const [inputValue, setInputValue] = React.useState("");
    const [month, setMonth] = React.useState(new Date());

    const dateValue = columnFilterValue ? new Date(columnFilterValue) : null;

    // Sync input value with filter value
    React.useEffect(() => {
      if (columnFilterValue && isValid(new Date(columnFilterValue))) {
        setInputValue(format(new Date(columnFilterValue), "dd-MM-yyyy"));
    ...
    return (
      <div className="space-y-1.5">
        ...
        <div className="relative">
          <Input
            className="pl-3 pr-20 h-9 text-sm"
            value={inputValue}
...
export function DataTable({
  columns,
  data,
  showFilters = null, // Can be: null (no filters), true (all filters), or array of column IDs
}) {
  ...
  // Determine which columns should show filters
  const filterableColumns = React.useMemo(() => {
    ...
    // If showFilters is null or false, return empty array
    if (!showFilters) return [];

    // If showFilters is true, return all filterable columns
    if (showFilters === true) return allColumns;

    // If showFilters is an array, filter columns by those IDs
    if (Array.isArray(showFilters)) {
      return allColumns.filter((column) => showFilters.includes(column.id));
    }
                ...
                <TableRow>
                  ...
                  >
                    <div className="empty-state py-8 md:py-12">
                      <p className="empty-state-title text-sm md:text-base">
                        No results found
                      </p>
